...
Engine forwards `IMeetingLiveStreamCtrlEvent::onRawLiveStreamPrivilegeRequestTimeout`, which fires reliably at ~30 s after `RequestRawLiveStreaming`. Phase B-Extended diagnostics established that no SDK callback or queryable state change signals host actions on the popup post-timeout — the host may still grant successfully (handled via the existing `Changed(true)` callback) but a Deny click or popup dismissal is invisible to the SDK from our side. The plugin transitions `g_rawPrivilegeState` to `TimedOut` on this signal, which renders a "Still waiting for permission, please check with host" message — honest about the uncertainty rather than asserting a denial. Engine and plugin both suppress this signal when state is already `Granted` — the SDK has been observed firing the timeout 10–25 s after a successful grant; this defence remains.
