...
public:
    ...
    virtual void onUserAudioStatusChange(
        ...
        // An audio status change is also how we learn this client joined
        // computer audio, which a pending isolated-audio subscribe waits for.
        IsolatedAudioOnAudioStatusChanged();
        if (!lst) return;
...
public:
    ...
    virtual void onUserRawLiveStreamingStatusChanged(
        ...
        if (selfPresent) {
            ...
            // Raw data is live: per-participant audio can be subscribed now
            // (no-op once subscribed).
...
public:
    ...
    // level: ConnectionQuality (0 Unknown .. 6 Excellent).
