...
//    "display_name":"<optional session-only name override or empty>"}
//
// Starts a fresh instant meeting with the current user as host. No password,
// no meeting number to look up — the SDK provisions the meeting on the fly
// using the user's ZAK. The meeting number returned by GetMeetingInfo() in
// the existing MEETING_STATUS_INMEETING callback is what the plugin shows
// to the user for sharing.
//
// SDK call: IMeetingService::Start(StartParam&) with userType =
// SDK_UT_WITHOUT_LOGIN. meetingNumber=0 is the SDK convention for "start
// an instant meeting"; the SDK will allocate one against the user's
// account (typically their PMI).
// ---------------------------------------------------------------------------
void HandleCreateInstantMeeting(const std::string& json) {
    ...
    // Storage lifetimes must outlive Start() — static, same pattern as Join.
    // Distinct names from the Join() statics so concurrent reuse is safe.
    ...
    if (startErr != ZOOM_SDK_NAMESPACE::SDKERR_SUCCESS) {
        ...
        sprintf_s(buf,
            "{\"type\":\"meeting_failed\",\"code\":%d,"
            "\"message\":\"Could not create instant meeting. SDK error: %d\"}",
